...
parser.add_argument('--overview-eye', nargs=3, type=float)
parser.add_argument('--overview-target', nargs=3, type=float)
...
if args.overview_eye:
    config['overview_eye'] = args.overview_eye
    config['overview_eye'][2] = min(config['overview_eye'][2], 2.3)
if args.overview_target:
    config['overview_target'] = args.overview_target
...
# Recorded poses drive a USD visual copy; an active articulation can retain stale USD transforms without a physics step.
UsdGeom.Imageable(robot.prim).MakeInvisible()
visual = UsdGeom.Xform.Define(stage, '/World/CarterReplayVisual')
visual.GetPrim().GetReferences().AddReference(str(ROOT/'assets/isaac-4.0/Isaac/Robots/Carter/nova_carter.usd'))
from pxr import Usd
...
visual.ClearXformOpOrder()
visual_position = visual.AddTranslateOp()
visual_orientation = visual.AddOrientOp(UsdGeom.XformOp.PrecisionDouble)
world.stop()
...
def render_human_frame(sim_seconds):
    ...
    human_position[2] = human_state.get('floor_z', 0.)
...
def set_view(view_name, carter):
    ...
    if view_name == 'Carter camera':
        sensor=manifest['resolved'].get('sensor', {'height_m':.6, 'forward_offset_m':.25,
            'horizontal_fov_deg':120., 'vertical_fov_deg':100.})
        yaw=carter['heading']
...
try:
    for index, row in enumerate(rows):
        ...
        selected_index = round(args.snapshot*30) if args.snapshot is not None else 330
        if (args.check_only or args.snapshot is not None) and abs(index-selected_index) > 1:
            continue
        if index % 2:
            continue
        yaw = carter['heading']
        ...
        visual_position.Set(Gf.Vec3d(*position))
        visual_orientation.Set(Gf.Quatd(float(quaternion[0]),Gf.Vec3d(*map(float,quaternion[1:]))))
        for _ in range(4):
            world.render()
        observed = np.array(visual.ComputeLocalToWorldTransform(Usd.TimeCode.Default()).ExtractTranslation())
        ...
        people = row.get('people', [person])
        actor_fields = []
        for actor_id, actor in enumerate(people):
            human_present = actor['present']
            if not human_present:
                continue
            pos = np.array([actor['x'], actor['y']])
            # Use the complete recorded prefix when previewing a sparse frame.
            if actor_id not in actor_previous:
                distance = 0.
            else:
                distance = np.linalg.norm(pos-actor_previous[actor_id])
            actor_previous[actor_id] = pos
            actor_distances[actor_id] = actor_distances.get(actor_id, 0.)+distance
            gait_cycles = actor_distances[actor_id]/walk['stride_m']
            human_state = {'xy':pos, 'speed':actor['speed'], 'floor_z':actor.get('floor_z',0.)}
            human_yaw = actor['heading']
            actor_fields.append(render_human_frame(row['timestamp']))
        for field in fields:
            setattr(model, field, torch.cat([static_fields[field]]+[a[field] for a in actor_fields], dim=0))
        for view_name in view_names:
            set_view(view_name,carter)
            gs_rgb, gs_alpha, gs_depth = render_scene()
            ...
            if args.snapshot is not None and view_name=='Overview':
                imageio.imwrite(str(OUT/f'{args.name}_native.png'),rgb)
                print('NATIVE_DIAGNOSTIC',observed,camera.get_world_pose(),
                    UsdGeom.BBoxCache(Usd.TimeCode.Default(),['default']).ComputeWorldBound(visual.GetPrim()).ComputeCentroid(),
                    np.nanmin(mesh_depth),np.nanmax(mesh_depth),flush=True)
        ...
        if args.snapshot is not None:
            imageio.imwrite(str(OUT/f'{args.name}.png'), composite)
        frame_count += 1
        if args.snapshot is not None or args.check_only:
            break
        if frame_count % 45 == 0:
    ...
    writer.close()
    import hashlib
    result={'success':failure is None and frame_count == (1 if args.check_only or args.snapshot is not None else len(rows[::2])),
