...
parser.add_argument('--name', default='carter_scene55')
...
camera.set_horizontal_aperture(48*math.tan(math.radians(85)/2))
...
gs_camera = GSCamera(0, w2c[:3, :3].T, w2c[:3, 3], math.radians(85),
                     2*math.atan(height/width*math.tan(math.radians(85)/2)),
...
def capture(index, center, speed, reached):
    ...
    focal = width/(2*math.tan(math.radians(85)/2))
    ...
    cv2.rectangle(composite, (12, 12), (750, 68), (28, 28, 28), -1)
    text = f'Carter | goal distance {np.linalg.norm(route[-1]-center):.2f} m | speed {speed:.2f} m/s'
    cv2.putText(composite, text, (24, 46), cv2.FONT_HERSHEY_SIMPLEX, .65, (240, 240, 240), 1, cv2.LINE_AA)
    if writer is not None:
        writer.append_data(composite)
    if index == 0 or reached:
        cv2.imwrite(str(OUT / (args.name + ('_goal.png' if reached else '_start.png'))), cv2.cvtColor(composite, cv2.COLOR_RGB2BGR))
...
try:
    ...
    if writer is not None:
        writer.close()
    result = {'success': failure is None and stopped_steps >= 240 and trajectory[-1]['goal_distance'] < .12,
