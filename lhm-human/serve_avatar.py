...
def main():
    ...
    renderer.cuda().eval()
    state_name = 'habitat_state.pt' if args.habitat else ('walking_state.pt' if args.walking else 'avatar_state.pt')
    ...
    if args.export:
        ...
        output_name = 'habitat_motion.npy' if args.habitat else ('walking_motion.npy' if args.walking else 'human_motion.npy')
        np.save(ROOT / 'output' / output_name, np.stack(frames))
        print('EXPORTED_MOTION', count, flush=True)
        return
    endpoint = ROOT / 'output/avatar.sock'
    ...
    try:
        ...
        with connection:
            while True:
                ...
                frame_index = struct.unpack('<I', request)[0]
        ...
        if endpoint.is_socket():
            endpoint.unlink()
        (ROOT / 'output/avatar_service_result.json').write_text(json.dumps({'frames':served, 'mean_seconds':sum(times)/len(times) if times else None}))
